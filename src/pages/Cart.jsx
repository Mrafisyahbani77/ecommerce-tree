...
export default function Cart() {
  ...
  const { mutate: createOrderAndPay, isLoading: buttonLoading } =
    useCreateOrderAndPay();
      ...
      createOrderAndPay(cartItemId, {
        onSuccess: (snapToken) => {
          if (!isPopupOpen) {
            ...
            alert(
              "Payment popup is already open. Please complete the current payment process first."
            );
          }
        },
        onError: (error) => {
      ...
      <div className="font-bold text-center flex items-center justify-center min-h-screen max-w-2xl container">
        <div>
          <h1 className="text-xl">
            Anda Tidak Bisa Membuka Halaman ini sebelum Login
          </h1>
          <Link
            to="/login"
            className="mt-5 block text-white rounded-lg bg-yellow-300 py-3 px-5 text-lg"
          >
            Login Disini
          </Link>
        </div>
      ...
      <div className="container mx-auto px-4 md:px-10">
        ...
        <div className="flex flex-wrap justify-between items-start md:items-start space-y-4 md:space-y-0 md:space-x-4">
                      ...
                      >
                        ...
                        <div className="flex items-center w-full">
                          <img
                            ...
                            className="md:w-24 md:h-24 w-20 h-16 object-cover rounded-md mr-4"
                          />
                          ...
                          <p className="ml-auto text-right md:text-lg kecil text-red-400">
          ...
          <div className="bg-white shadow-md rounded-lg p-6 md:max-w-lg w-full md:w-auto">
            ...
            <button
              className="mt-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md w-full"
              onClick={handleButtonClick}
              disabled={buttonLoading || isPopupOpen}
            >
              {buttonLoading ? (
                <svg
